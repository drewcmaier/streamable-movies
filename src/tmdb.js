...
export async function getStreamingServices(titles) {
  ...
  for (const title of titles) {
    ...
    const usResults = providersRes.results?.US;
    if (!usResults) {
      continue;
    }

    const usProviders = [...(usResults.flatrate ?? []), ...(usResults.ads ?? []), ...(usResults.free ?? [])];
    const available = usProviders.map((prov) => prov.provider_name);
